...
void MainWindow::on_empezarPB_clicked()
{
    ...
    if (database.open())
    {
        ...
        if (qry2.exec("SELECT id_mesa FROM mesas WHERE id_mesa = '"+mesa+"';"))
        {
            //cout<<"Aqui toy"<<endl;
            int cont = 0;
            while(qry2.next())
            ...
            if(cont == 1)
            {
                ...
                {
                    QMessageBox succes;
                    succes.setText("¡No se pudo iniciar sesión... Error crítico!");
        ...
        {
            QMessageBox warning;
            warning.setText("¡Error crítico con la base de datos... !");
            warning.setIcon(QMessageBox::Warning);
            warning.exec();
        }
