...
void CRUDGerenteWidget::on_PBNuevoMesa_clicked()
{
    ui->LEIDMesa->setEnabled(true);

    ui->PBGuardarMesa->setEnabled(false);
    ui->PBCancelarMesa->setEnabled(true);
    ui->PBNuevoMesa->setEnabled(false);
    ui->PBEditarMesa->setEnabled(false);
    ui->PBEliminarMesa->setEnabled(false);

}

void CRUDGerenteWidget::on_PBGuardarMesa_clicked()
{
    if (!editedMesa)
    {
        //Agregar
        ...
        ui->LEIDMesa->setEnabled(false);


        ui->PBGuardarMesa->setEnabled(false);
        ui->PBCancelarMesa->setEnabled(false);
        ui->PBNuevoMesa->setEnabled(true);

        if (database.open())
        {
            QSqlQuery qry;

            QString id = ui->LEIDMesa->text();
            QString estatus = ui->CBEstatusMesa->currentText();

            qry.prepare("INSERT INTO mesas ("
                        "id_mesa,"
                        "estatus)"
                        " VALUES (?,?);");
            qry.addBindValue(id);
            qry.addBindValue(estatus);
            ...
            if (qry.exec())
            {
                ...
                //Clear Line Edits
                ui->LEIDMesa->clear();
                ui->CBEstatusMesa->setCurrentIndex(0);
    ...
    else
    {
        ...
        ui->LEIDMesa->setEnabled(false);
        ui->CBEstatusMesa->setEnabled(false);
        ...
        if (database.open())
        {
            ...
            QString id_buscar = ui->LEBuscarMesa->text();
            QString id = ui->LEIDMesa->text();
            QString estatus = ui->CBEstatusMesa->currentText();
...
void CRUDGerenteWidget::on_PBCancelarMesa_clicked()
{
    //Clear Line Edits
    ui->LEIDMesa->clear();
    ui->CBEstatusMesa->setCurrentIndex(0);
...
void CRUDGerenteWidget::on_PBEditarMesa_clicked()
{
    ui->PBGuardarMesa->setEnabled(true);
    ui->PBCancelarMesa->setEnabled(true);
    ui->PBNuevoMesa->setEnabled(false);
    ui->PBEditarMesa->setEnabled(false);
    ui->PBEliminarMesa->setEnabled(false);
    ...
    ui->LEIDMesa->setEnabled(true);
    ui->CBEstatusMesa->setCurrentIndex(0);
    ui->CBEstatusMesa->setEnabled(true);

    editedMesa = true;
}

void CRUDGerenteWidget::on_PBEliminarMesa_clicked()
{
    //Eliminar
...
void CRUDGerenteWidget::on_PBBuscarMesa_clicked()
{
    QSqlDatabase database;
    database = QSqlDatabase::addDatabase("QSQLITE");
    database.setDatabaseName(rootDataBase);
    ...
    if (database.open())
    {
        ...
        QString id = ui->LEBuscarMesa->text();

        if (qry.exec("SELECT id_mesa, estatus FROM mesas WHERE id_mesa ='"+ id +"';"))
        {
            int cont = 0;
            while(qry.next())
            {
                ...
                QString id = qry.value(0).toString();
                QString estatus = qry.value(1).toString();

                ui->LEIDMesa->setText(id);
                ui->CBEstatusMesa->setCurrentText(estatus);

                ui->PBGuardarMesa->setEnabled(false);
                ui->PBCancelarMesa->setEnabled(false);
                ui->PBNuevoMesa->setEnabled(false);
                ui->PBEditarMesa->setEnabled(true);
                ui->PBEliminarMesa->setEnabled(true);
...
void CRUDGerenteWidget::on_LEBuscarMesa_textChanged(const QString &arg1)
{
    if(arg1.length() >= 1)
    {
        ui->PBBuscarMesa->setEnabled(true);

        if (ui->LEIDMesa->text().length() != 0)
        {
            ui->LEIDMesa->clear();
            ui->CBEstatusMesa->setCurrentIndex(0);
    ...
    {
        ui->PBBuscarMesa->setEnabled(false);

    }
}

void CRUDGerenteWidget::on_LEIDMesa_textChanged(const QString &arg1)
{
    Q_UNUSED(arg1);
    enableMesas();
}

void CRUDGerenteWidget::enableMesas()
{
    if(ui->LEIDMesa->text().length() > 0)
    {
        ui->PBGuardarMesa->setEnabled(true);
